...
class simulation:
    ...
    def loop_500(self, verbose, output):
        ...
        if self.system == "o":
            self.minp[1:] = self.minp[1:] + self.min[1:]
        ...
        if output:
            ...
            for k in range(1, self.nm+1):
                if self.system == "c":
                    ...
                    if self.lmin[k] == 1 and self.lmin0[k] == 0:
                        lines.append("start of precipitation of {} at fc = {} : moles = {}".format(self.mineral_S[k], self.fc, self.minp[k]))
                    elif self.lmin[k] == 0 and self.lmin0[k] == 1:
                        lines.append("end of precipitation of {} at fc = {} : moles = {}".format(self.mineral_S[k], self.fc, self.minp[k]))
            with open(self.event_file, 'a') as file:
                if len(lines) > 0:
                    for line in lines:
                        file.write(line)
    ...
    def reseq(self, verbose, output):
        ...
        while self.nu != 0:
            ...
            for i in range(13, self.n+1):
                ...
                for j in range(1, self.n+1):
                    if j != 11:
                        self.kmat[i, 0] = self.kmat[i, 0] + self.kmat[i, j]
