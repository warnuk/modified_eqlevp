    ...
    private:
        ...
        void run_evp() {
            ...
            while (true) {
                ...
                for (int k=1; k<=nm; k++) {
                    if (psol[k] == 1.0e50 and linvar[k] == 0) {
                        psol[k] = psol0[k];
                        if (verbose == 1) {
                ...
                for (int i=1; i<=10; i++) {
                    for (int k=1; k<=nm; k++) {
                        if (system == "c") totest[i] += min[k] * wmin[k][i];
                        if (system == "o") totest[i] += minp[k] * wmin[k][i];
                    }
                }
                for (int j=1; j<=ncm; j++) {
                    for (int k=1; k<=nm; k++) {
                        if (system == "c") totest[11] += wmin[k][j] * kmat[11][j] * min[k];
                        if (system == "o") totest[11] += wmin[k][j] * kmat[11][j] * minp[k];
                    }
                }
                totest[11] += mwev * 2;
                evp_density();
                ...
                fc = mh2o / mol[11];
                alk = molal[12] - molal[12] + molal[14] * 2 + molal[15] + (molal[16] + molal[17]) * 2;
                alk += molal[18] + molal[19] + molal[20] + molal[21] * 2;
                alk += molal[22] + molal[23] + molal[24] - molal[25];
                ...
                for (int k=1; k<=nm; k++) {
                    if (system == "c") {
                        hco3 += wmin[k][15] * min[k];
                        co3 += wmin[k][14] * min[k];
                    } else if (system == "o") {
                        hco3 += wmin[k][15] * minp[k];
                        co3 += wmin[k][14] * minp[k];
                    }
                }
                ...
                if (ncpt == 1 or ncpt % output_step == 0) {
                    if (verbose == 1) {
                        ...
                        cout << setw(22) << "ionic strength      = " << setw(18) << fi << setw(23) << "salinity (g/kg)      = " << setw(17) << std << endl;
                ...
                if (output == 1 and (ncpt == 1 or my_S != my0_S or ncpt % print_step == 0)) {
                    ...
                    dumpfile << "salinity                 = " << std/1000 << " g/kg" << endl;
                ...
                if (output == 1) {
                    ...
                    if (ncpt == 1 or my_S != my0_S or npasf == output_step) {
                        ...
                        outfile.open(chem_file, ios::app);
                            ...
                            outfile << fc << "," << mwev << "," << dens << "," << -log10(act[13]) << ",";
                            outfile << alk * ee << ",";
                            for (int i=1; i<=10; i++) {
                                if (tot[i] > 0) {
                                    s = 0;
            ...
            tot[11] -= 2 * mol[11] * xi;
            kinvariant = 0;
            lmin[ksupprim] = 0; min[ksupprim] = 0; psol[ksupprim] = 1.0e50;
            min0[ksupprim] = 0;
            if (verbose == 1) {
        ...
        void evp_actp() {
            ...
            gact[16] = 1; gact[17] = 1; gact[11] = aw / mh2o;
            ndepact = 1;
        }
        ...
        void evp_density() {
            ...
            if (ncpt == 1) {
                for (int i=0; i<=ncdens; i++) {
                    vector<double> temp;
                    for (int k=0; k<=nadens; k++) temp.push_back(0); au.push_back(temp);
                }
                for (int i=0; i<=ncdens; i++) {
                    vector<double> temp;
                    for (int k=0; k<=nadens; k++) temp.push_back(0); bu.push_back(temp);
                }
                infile.open("densite");
                for (int i=1; i<=5; i++) {
                    for (int j=1; j<=5; j++) {
        ...
        void evp_invar() {
            double swap, ah2o;
            int ninvar, l, n1, n2, det, nz, ii, jj, kk, det1, n3, n4, i1, i2, ncond;
            ...
            winv[2][13] = 1; winv[2][11] = -2; winv[2][14] = 1;
        ...
        void reseqinv() {
            int nt, ncomp, j, i, ii, ncond, i1, i2, kk, nmin;
        ...
        void peritec() {
            double u, s, swap;
            int nt, k, i, j, l, ni, nj, ncomp, i1, i2, ii;
            string x_S;
